...
> **Last verified: 2026-08-24** (web-search-verified against provider docs and independent trackers on this date). Free tiers change frequently — providers add credit-card requirements, cut rate limits, or deprecate models with little notice. Treat every row below as "true as of last verification," not a permanent guarantee. Re-check a provider's own pricing/docs page before depending on it in production.
>
...
2. [Groq deprecation watch](#groq-deprecation-watch-verified-2026-08-24-primary-source)
...
Rows marked **🔑 have key** indicate providers commonly configured in **MixMaster** via `.env` (see [`.env.example`](../.env.example)). When copying this doc into another project, treat 🔑 as a reminder to inventory your own API keys.
...
| Provider | Free, no card? | Current free-tier models (verified 2026-08-24) | Notes |
|---|---|---|---|
...
| **GroqCloud** 🔑 | **Yes** — no card | `openai/gpt-oss-120b`, `openai/gpt-oss-20b`, `qwen/qwen3.6-27b` | Extremely fast inference (LPU hardware). See [deprecation watch](#groq-deprecation-watch-verified-2026-08-24-primary-source) below — Groq deprecates models aggressively. ~14,400 req/day, 30 RPM typical on free tier (varies per model). |
...
| **Cloudflare Workers AI** | **Yes** — no card, starts immediately on any free Cloudflare account | 50+ models incl. Llama, Mistral, Gemma, DeepSeek, Qwen, Whisper (speech), SDXL (image) | 10,000 free "Neurons" per day. **Not wired in MixMaster** — would need Cloudflare account ID + API token. |
...
### Groq deprecation watch (verified 2026-08-24, primary source)

Fetched directly from `https://console.groq.com/docs/deprecations` on the verification date above. **Llama 3.1 8B Instant and Llama 3.3 70B Versatile shut down 2026-08-16** — migrate immediately if still referenced in code or env.

| Deprecated model | Shutdown date | Recommended replacement |
|---|---|---|
| `llama-3.1-8b-instant` | **2026-08-16 (completed)** | `openai/gpt-oss-20b` |
| `llama-3.3-70b-versatile` | **2026-08-16 (completed)** | `openai/gpt-oss-120b` or `qwen/qwen3.6-27b` |
| `qwen/qwen3-32b` | **2026-07-17 (completed)** | `openai/gpt-oss-120b` |
| `meta-llama/llama-4-scout-17b-16e-instruct` | 2026-07-17 (completed) | `openai/gpt-oss-120b` or `qwen/qwen3.6-27b` |
| `moonshotai/kimi-k2-instruct-0905` | 2026-04-15 (completed) | `openai/gpt-oss-120b` |
| `meta-llama/llama-4-maverick-17b-128e-instruct` | 2026-03-09 (completed) | `openai/gpt-oss-120b` |
| `meta-llama/llama-guard-4-12b` | 2026-03-05 (completed) | `openai/gpt-oss-safeguard-20b` |

**MixMaster implementation:** [`src/lib/admin/ai-provider-models.ts`](../src/lib/admin/ai-provider-models.ts) defines the Groq chain (`openai/gpt-oss-20b` → `openai/gpt-oss-120b` → `qwen/qwen3.6-27b`). Orchestration lives in [`src/lib/admin/ai-composer-fill.ts`](../src/lib/admin/ai-composer-fill.ts).
...
MixMaster (cocktail-mixer) implements this pattern for the **admin newsletter composer assist** API:

- [`src/lib/admin/ai-provider-models.ts`](../src/lib/admin/ai-provider-models.ts) — provider registry (Layer 1): ordered chains for Groq, Gemini, OpenRouter, and optional Hugging Face Inference Router.
- [`src/lib/admin/ai-composer-fill.ts`](../src/lib/admin/ai-composer-fill.ts) — generic OpenAI-compatible client + Gemini native client (Layer 2), inner model loops, 429 fast-skip, outer provider orchestration (Layer 3).

**Provider order:** Groq → Gemini → OpenRouter → Hugging Face (skipped when `HUGGINGFACE_API_KEY` is unset).

**Default model chains (Aug 2026):**

| Provider | Models tried in order |
|---|---|
| Groq | `openai/gpt-oss-20b`, `openai/gpt-oss-120b`, `qwen/qwen3.6-27b` |
| Gemini | `gemini-2.5-flash`, `gemini-2.5-flash-lite` |
| OpenRouter | `openai/gpt-oss-20b:free`, `deepseek/deepseek-chat-v3-0324:free`, `meta-llama/llama-3.3-70b-instruct:free` |
| Hugging Face | `openai/gpt-oss-20b:fastest`, `openai/gpt-oss-120b:fastest`, `Qwen/Qwen2.5-7B-Instruct:fastest` |

Env overrides (`GROQ_MODEL`, `GEMINI_MODEL`, `OPENROUTER_MODEL`, `HUGGINGFACE_MODEL`) replace the chain with a single model when set.
